...
def get_last_block_height(order: int) -> int:
...
def get_pyramid_height(order: int) -> int:
    ...
    if order <= 0:
        return -1
    return int((order + 3) * (order + 4) / 2) - 3
...
def get_bottom_width(order: int) -> int:
    ...
    # 2. Normal growth
    normal_growth = (get_pyramid_height(order) - 1) * 2
    ...
    return c + normal_growth + block_offsets
...
def build_pyramid(order: int) -> str:
    ...
    pyramid = []  # A pyramid is a list of strings
    ...
    for block in range(order):
        for block_line in range(block_height):
            ...
            line = build_line(beginning_spaces-6, star_count, door_width, has_doorknob)
            pyramid.append(line)
...
def test_get_last_block_height():
    assert get_last_block_height(0) == -1
    assert get_last_block_height(1) == 3
    assert get_last_block_height(2) == 4
    assert get_last_block_height(3) == 5
    assert get_last_block_height(4) == 6
    assert get_last_block_height(5) == 7
    assert get_last_block_height(10) == 12
    assert get_last_block_height(136) == 138

    print(f"Test: {'get_last_block_height'.ljust(25)} - {GREEN}OK{ENDC}")


def test_get_pyramid_height():
    assert get_pyramid_height(0) == -1
    assert get_pyramid_height(1) == 7
    assert get_pyramid_height(2) == 12
    assert get_pyramid_height(3) == 18
    assert get_pyramid_height(4) == 25
    assert get_pyramid_height(5) == 33
    assert get_pyramid_height(6) == 42
    assert get_pyramid_height(7) == 52
    assert get_pyramid_height(12) == 117
    assert get_pyramid_height(87) == 4092
...
def test_get_bottom_width():
    ...
    assert get_bottom_width(4) == 49
    assert get_bottom_width(5) == 67
...
def test_get_beginning_spaces():
    ...
    assert get_beginning_spaces(5) == 33
    assert get_beginning_spaces(6) == 44
